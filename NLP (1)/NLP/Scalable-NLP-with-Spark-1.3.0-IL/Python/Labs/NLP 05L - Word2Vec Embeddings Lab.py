...
# TODO
(trainDF, testDF) = wvDF.randomSplit([0.8,0.2], seed=40)
...
# Create the model
lr_embeddings = LogisticRegression(featuresCol="word2vecEmbedding",labelCol="ExcellentReview")
...
val_acc = lrModel_embeddings.transform(testDF).where("ExcellentReview=prediction").count()/testDF.count()
