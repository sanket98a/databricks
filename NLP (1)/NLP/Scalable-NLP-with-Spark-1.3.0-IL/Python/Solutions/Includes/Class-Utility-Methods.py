...
def classroomCleanup(
    ...
    for stream in spark.streams.active:
        ...
        queries = list(
            filter(lambda query: query.name == stream.name, spark.streams.active)
        )
    ...
    try:
        tables = (
            spark.sql("show tables from {}".format(database))
            .select("tableName")
            .collect()
        )
        for row in tables:
    ...
    if len(actions) == 0:
        htmlMsg += "no actions taken."
    else:
        htmlMsg += f"<ul>{actions}</ul>"
    displayHTML(htmlMsg)

    if dropDatabase:
        daLogger.logEvent("Classroom-Cleanup-Final")
    else:
        daLogger.logEvent("Classroom-Cleanup-Preliminary")
...
.multiple_choicex.checkedRed {
  ...
  """
    ...
    return (
        html.replace("GET_MODULE_NAME", getModuleName())
