...
train_predction=lrModel.transform(trainDF).select("prediction","ExcellentReview")
test_predction=lrModel.transform(testDF).select("prediction","ExcellentReview")
...
sns.heatmap(confusion_matrix(train_predction.select("ExcellentReview").toPandas(),train_predction.select("prediction").toPandas()),annot=True)

# COMMAND ----------

sns.heatmap(confusion_matrix(test_predction.select("ExcellentReview").toPandas(),test_predction.select("prediction").toPandas()),annot=True)
