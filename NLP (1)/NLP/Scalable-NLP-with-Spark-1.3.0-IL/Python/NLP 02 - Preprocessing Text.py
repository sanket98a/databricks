...
# MAGIC * [SparkML](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html)/MLlib
...
textDF = spark.read.parquet("/mnt/training/reviews/reviews_cleaned.parquet")
...
# using SparkML
spark_df = tokenizer.transform(textDF)

# COMMAND ----------

# MAGIC %md
# MAGIC To achieve the same result using Python's `.split()` in series, we have to first convert our Spark DataFrame to a Pandas DataFrame which we call `textPDF`. Run the next cell to do this.

# COMMAND ----------

spark.conf.set("spark.sql.execution.arrow.enabled", "true")  # to speed up toPandas() conversion
textPDF = textDF.toPandas()
...
textPDF["Tokens"] = textPDF["Text"].apply(lambda text: text.split())
...
display(tokenizedDF.withColumn("token", explode(col("Tokens"))).groupBy("token").count().sort(col("count").desc()).limit(5))
...
display(wordDist.limit(30))

# COMMAND ----------

# %fs  ls "dbfs:/user/sanket.bodake@affine.ai/nlp/nlp_01_nlp_course_overview/reviews-cleaned.delta"
...
# MAGIC
# MAGIC Steps:
# MAGIC 1. [HashingTF](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.feature.HashingTF) or [CountVectorizer](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.feature.CountVectorizer)
...
# MAGIC 2. [IDF](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.feature.IDF)
...
# MAGIC 3. [Normalizer](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.feature.Normalizer)
...
# MAGIC
# MAGIC Note: Since our TF-IDF vectors will have a lot of zero entries (at indices where a token is not present), Spark stores and displays these vectors as [SparseVectors](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.linalg.SparseVector) to save space.
...
hashingTF = HashingTF(inputCol="CleanTokens", outputCol="TermFrequencies")
new=hashingTF.transform(processedDF)


idf = IDF(inputCol="TermFrequencies", outputCol="TFIDFScore")
i=idf.fit(new)
display(i.transform(new))
...
# save DF with tfidf for use in the UDF notebook
path = f"{workingDir}/tfidf.delta"
tfidfDF.repartition(8).write.mode("OVERWRITE").format("delta").save(path)
