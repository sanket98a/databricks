...
def get_wine_data():
    ...
    """
    # 1. read in data
    data_path = f"{root_working_dir.replace('dbfs:', '/dbfs')}/datasets"
    white_wine = pd.read_csv(f"{data_path}/winequality-white.csv", sep=";")
    red_wine = pd.read_csv(f"{data_path}/winequality-red.csv", sep=";")
