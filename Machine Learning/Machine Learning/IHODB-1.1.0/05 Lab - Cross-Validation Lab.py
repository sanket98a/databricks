...
# MAGIC %sql
# MAGIC -- TODO
# MAGIC SELECT  steps_8000,count(*) FROM ihodb.ht_user_metrics_cv_lab GROUP BY steps_8000
...
train_df, test_df = train_test_split(ht_user_metrics_pd_df, train_size=0.85, test_size=0.15, random_state=42)

train_df, cross_val_df = train_test_split(train_df, train_size=0.85, test_size=0.15, random_state=42)
...
# TODO
len(parameter_grid['max_depth'])*len(parameter_grid['n_estimators'])*len(parameter_grid['min_samples_split'])*\
len(parameter_grid['max_features'])*len(parameter_grid['max_samples'])
...
# TODO
grid_search.fit(train_df.drop("steps_8000", axis=1), train_df["steps_8000"])
...
# MAGIC %md
# MAGIC - Test Set accuracy is find performance of model on new unknown data which is model is not analyzed previsuly.
