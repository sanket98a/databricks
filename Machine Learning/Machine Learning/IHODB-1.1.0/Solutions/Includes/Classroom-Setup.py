...
spark.sql(f"CREATE DATABASE IF NOT EXISTS {courseName}")
spark.sql(f"USE {courseName}")

# COMMAND ----------

def create_table(courseName, tableName, dataPath, overwrite = False):
  df = spark.read.csv(dataPath, header = True)
  if overwrite:
    df.write.format("delta").mode("overwrite").save(f"/{courseName}/{tableName}")
  elif not overwrite:
    df.write.format("delta").save(f"/{courseName}/{tableName}")
  spark.sql(
    f"CREATE TABLE IF NOT EXISTS {courseName}.{tableName} USING DELTA LOCATION '/{courseName}/{tableName}'"
  )

# COMMAND ----------

create_table(courseName, "ht_users", "/mnt/training/healthcare/tracker/moocs/users.csv", True)
create_table(courseName, "ht_daily_metrics", "/mnt/training/healthcare/tracker/moocs/daily-metrics.csv", True)

# COMMAND ----------

# MAGIC %fs rm -r /ihodb/ht-users-athletes

# COMMAND ----------

# MAGIC %fs rm -r /ihodb/ht-users-non-athletes

# COMMAND ----------

# MAGIC %fs rm -r /ihodb/ht-device-metrics

# COMMAND ----------

# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS ihodb.ht_users_athletes;
# MAGIC DROP TABLE IF EXISTS ihodb.ht_users_non_athletes;
# MAGIC DROP TABLE IF EXISTS ihodb.ht_device_metrics;
# MAGIC